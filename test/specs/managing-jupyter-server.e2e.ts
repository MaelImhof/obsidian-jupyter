...
describe('Managing Jupyter server state', async () => {
    ...
    it('can be done through a ribbon icon', async () => {
        ...
        const idleServerStatus = browser.$('.side-dock-ribbon-action[aria-label="Start Jupyter Server"]');
        ...
        const runningServerStatus = browser.$('.side-dock-ribbon-action[aria-label="Stop Jupyter Server"]');
    ...
    it('displays status update notices when state changes', async () => {
        ...
        const statusUpdateNotice = await browser.$$('.notice-container .notice .notice-message').find(async el => {
        ...
        const runningNotice = await browser.$$('.notice-container .notice .notice-message').find(async el => {
            const text = await el.getText();
        ...
        const stoppedNotice = await browser.$$('.notice-container .notice .notice-message').find(async el => {
            const text = await el.getText();
