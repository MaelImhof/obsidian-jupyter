...
import { addIcon, Menu, MenuItem, Notice, PaneType, TAbstractFile, TFile, TFolder, WorkspaceLeaf } from "obsidian";
...
export class CreateNotebooksFeature implements IFeature {
    ...
    async onload(plugin: JupyterForObsidian): Promise<void> {
        ...
        this.plugin.settingsProxy.on(
            "change:displayFolderContextMenuItem",
            (newVal, _oldVal) => {
                if (newVal) {
        ...
        this.plugin.addCommand({
            ...
            callback: (async () => {
                ...
                await this.createJupyterNotebook(
                    JupyterAbstractPath.fromRelative(
                        "/",
                        true,
            ...
            menu.addItem((item: MenuItem) => {
                item
                    ...
                    .onClick(async (_event: MouseEvent|KeyboardEvent) => {
                        await this.createJupyterNotebook(JupyterAbstractPath.fromRelative(file.path, true, this.plugin.app.vault));
                    });
            });
        }
    }
    ...
    private async createJupyterNotebook(folder: JupyterAbstractPath) {
        ...
        if (this.plugin.settings.openCreatedFileMode !== OpenCreatedNotebook.DONT) {
            ...
            leaf.openFile(
                this.plugin.app.vault.getFileByPath(
                    file.getRelativePath() as string
                ) as TFile
            );
        }
