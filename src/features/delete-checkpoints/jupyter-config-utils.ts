...
export async function customJupyterConfigExists(plugin: JupyterForObsidian): Promise<boolean> {
    ...
    return await plugin.app.vault.adapter.exists(normalizePath(configPath.getRelativePath() as string));
}
...
print("[Jupyter for Obsidian] Custom configuration of Jupyter for Obsidian loaded successfully.")`
	...
	await plugin.app.vault.adapter.write(normalizePath(configPath.getRelativePath() as string), configContent);
	return true;
}
