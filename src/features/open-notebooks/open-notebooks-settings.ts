				...
				.addToggle((toggle: ToggleComponent) =>
					toggle
						...
						.onChange(async (value: boolean) => {
							if (
								plugin.env.getStatus() === JupyterEnvironmentStatus.STARTING &&
								!value
							) {
	...
	tab.registerSetting({
		...
		display: (el: HTMLElement, plugin: JupyterForObsidian) => {
			new Setting(el)
				...
				.addToggle((toggle: ToggleComponent) => {
					toggle.setValue(plugin.settings.startJupyterAuto).onChange((value: boolean) => {
	...
	tab.registerSetting({
		...
		display: (el: HTMLElement, plugin: JupyterForObsidian) => {
			new Setting(el)
				...
				.addToggle((toggle: ToggleComponent) => {
					toggle.setValue(plugin.settings.useSimpleMode).onChange((value: boolean) => {
	...
	tab.registerSetting({
		...
		display: (el: HTMLElement, plugin: JupyterForObsidian) => {
			new Setting(el)
				...
				.addToggle((toggle: ToggleComponent) =>
					toggle.setValue(plugin.settings.useStatusNotices).onChange((value: boolean) => {
	...
	tab.registerSetting({
		...
		display: (el: HTMLElement, plugin: JupyterForObsidian) => {
			new Setting(el)
				...
				.addToggle((toggle: ToggleComponent) => {
					toggle.setValue(plugin.settings.debugConsole).onChange((value: boolean) => {
