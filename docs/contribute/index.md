...
- Adding images, icons or other types of graphic contribution (which I'm not very good at right now, so much appreciated).
...
1. **Open an issue first.** Write about what you want to do, so that you do not do everything to find out at the very end that it won't be accepted for some reason (not in the scope of the plugin, not maintanable, ...).
...
4. **Open a [pull request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request) from your fork** once you are done working on it. Please set `dev` as target. A pull request to `release` would not be accepted as is, all changes must go through `dev` first.
